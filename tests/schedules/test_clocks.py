...
class TestIntervalClock:
    ...
    def test_interval_clock_events(self):
        ...
        assert list(itertools.islice(c.events(), 3)) == [
            today.add(days=1),
            today.add(days=2),
            today.add(days=3),
        ]
    ...
    def test_interval_clock_events_with_after_argument(self):
        ...
        c = clocks.IntervalClock(timedelta(days=1), start_date=start_date)
        assert list(itertools.islice(c.events(after=after.add(hours=1)), 3)) == [
            after.add(days=1),
            after.add(days=2),
            after.add(days=3),
        ]
    ...
    def test_interval_clock_end_date(self):
        ...
        assert list(itertools.islice(c.events(after=start_date), 3)) == [
            start_date.add(days=1)
        ]
        assert (
            list(itertools.islice(c.events(after=pendulum.datetime(2018, 2, 1)), 3))
            == []
        )
    ...
    def test_interval_clock_respects_after_in_middle_of_interval(self):
        ...
        assert list(
            itertools.islice(c.events(after=start_date + timedelta(microseconds=1)), 2)
        ) == [start_date.add(hours=1), start_date.add(hours=2)]

    def test_interval_clock_doesnt_compute_dates_before_start_date(self):
        start_date = pendulum.datetime(2018, 1, 1)
        c = clocks.IntervalClock(start_date=start_date, interval=timedelta(hours=1))
        assert list(
            itertools.islice(c.events(after=pendulum.datetime(2000, 1, 1)), 3)
        ) == [start_date, start_date.add(hours=1), start_date.add(hours=2)]

    def test_interval_clock_respects_microseconds(self):
        start_date = pendulum.datetime(2018, 1, 1, 0, 0, 0, 1)
        c = clocks.IntervalClock(start_date=start_date, interval=timedelta(hours=1))
        assert list(
            itertools.islice(c.events(after=pendulum.datetime(2010, 1, 1)), 3)
        ) == [start_date, start_date.add(hours=1), start_date.add(hours=2)]
...
#         if serialize:
#             c = ScheduleSchema().load(c.serialize())
#         next_4 = list(itertools.islice(c.events(after=dt), 4))
#         # skip 2am
...
class TestCronClock:
    ...
    def test_cron_clock_when_start_date_doesnt_align_with_schedule(self):
        ...
        c = clocks.CronClock(every_day, start_date=start_date)
        assert list(itertools.islice(c.events(), 1)) == [
    ...
    def test_cron_clock_when_start_date_and_after_doesnt_align_with_schedule(self):
        ...
        c = clocks.CronClock(every_day, start_date=start_date)
        assert list(
            itertools.islice(c.events(after=pendulum.datetime(2050, 1, 1, 7, 0, 0)), 1)
        ) == [pendulum.datetime(2050, 1, 2, 0, 0, 0)]
    ...
    def test_cron_clock_respects_microseconds(self):
        ...
        c = clocks.CronClock(every_day, start_date=start_date)
        assert list(itertools.islice(c.events(), 1)) == [pendulum.datetime(2050, 1, 2)]
...
class TestOneTimeClock:
    def test_create_onetime_clock(self):
        schedule = clocks.OneTimeClock(start_date=pendulum.now("UTC"))
    ...
    def test_start_date_must_be_datetime(self):
        with pytest.raises(TypeError):
            clocks.OneTimeClock(start_date=None)

    def test_onetime_clock_events(self):
        """Test that default after is *now*"""
        start_date = pendulum.today("UTC").add(days=1)
        c = clocks.OneTimeClock(start_date)
        assert list(itertools.islice(c.events(), 3)) == [start_date]
        assert list(itertools.islice(c.events(), 1)) == [start_date]

    def test_onetime_clock_events_with_after_argument(self):
        start_date = pendulum.today("UTC").add(days=1)
        c = clocks.OneTimeClock(start_date)
        assert list(
            itertools.slice(c.events(after=start_date - timedelta(seconds=1)), 1)
        ) == [start_date]
        assert list(itertools.slice(c.events(after=start_date.add(days=-1)), 1)) == [
            start_date
        ]
        assert list(itertools.slice(c.events(after=start_date.add(days=1)), 1)) == []
